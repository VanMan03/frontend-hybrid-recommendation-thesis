          ...
          <tbody className="divide-y divide-gray-200">
            {users.map((user) => (
              <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-6 py-4 font-mono text-sm text-gray-900">#{user.id}</td>
                ...
                <td className="px-6 py-4">
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                    user.activityLevel === 'High' ? 'bg-green-100 text-green-700' :
                    user.activityLevel === 'Medium' ? 'bg-yellow-100 text-yellow-700' :
                    'bg-gray-100 text-gray-700'
                  }`}>
                    {user.activityLevel}
                  </span>
                </td>
                <td className="px-6 py-4 text-gray-900 font-semibold">{user.itineraryCount}</td>
